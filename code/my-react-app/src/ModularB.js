    ...
    if (transformStrategy === CoordinateTransforms.DECISION_TREE) {
        ...
        tree.levels.forEach((nodes, depth) => {
            ...
            rings.push({
                ...
                subspaceId: `TreeDepth${depth}`,
                ringIndex: depth,
                sectors, // This is now the number of nodes at this depth
                structure: result.structure,
...
export const calculatePointPositions = (
    ...
    try {
        ...
        if (transformStrategy === CoordinateTransforms.DECISION_TREE) {
            // Use the sector information directly from the decision tree
            transformedPoints.forEach((point, pointIndex) => {
                if (!point.nodeAssignments) {
                    console.warn(`No nodeAssignments for point ${point.Point_ID}`);
                    return;
                ...
                // Use the sector information with start and end angles
                const sector = assignment.sector;
                if (!sector) {
                    console.warn(`No sector info for point ${point.Point_ID} at depth ${ringIndex}`);
                    return;
                }
                ...
                // Convert to Cartesian coordinates relative to sector center
                const x = sector.centerX + Math.cos(angle) * radius;
                const y = sector.centerY + Math.sin(angle) * radius;

                if (isNaN(x) || isNaN(y)) {
                    console.warn(`Invalid position for point ${point.Point_ID} at node ${assignment.nodeId}`);
                    return;
                }
