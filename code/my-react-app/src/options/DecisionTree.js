...
class DecisionTreeNode {
    constructor(data, depth = 0, nodeId = 0) {
        ...
        this.angle = 0; // Angle span in circular viz
        this.startAngle = 0; // Starting angle
        this.radius = 0; // Radius for radial layout
    }
...
export class DecisionTree {
    ...
    _calculateAngles() {
        ...
        this.maxRadius = this.levels.length;
        this.levels.forEach((nodes, level) => {
            nodes.forEach(node => {
                node.radius = (level + 1) / this.maxRadius;
            });
        });
            ...
            if (level === 0) {
                ...
                nodes.forEach((node, index) => {
                    ...
                    if (parent) {
                        ...
                        const nodeSamples = node.samples;
                        const totalSamples = (parent.left?.samples || 0) + (parent.right?.samples || 0);
                        const proportion = totalSamples > 0 ? nodeSamples / totalSamples : 0.5;
                        node.angle = parentAngle * proportion;
                        node.startAngle = parent.startAngle + (parent.left === node ? 0 : (sibling?.angle || 0));
                        node.sectorIndex = index;
                    } else {
                        node.angle = 0.1;
                        node.startAngle = index * 0.1;
    ...
    _findParent(node) {
        for (let level = node.depth - 1; level >= 0; level--) {
            const parentLevel = this.levels[level];
...
export const transformDecisionTree = (points, options = {}) => {
    ...
    // Create a color scale for labels
    const uniqueLabels = options.labelsData && options.labelsData.labels
        ? Object.keys(options.labelsData.labels).concat('unlabeled')
        : ['unlabeled'];
    const colorScale = d3.scaleOrdinal()
        .domain(uniqueLabels)
        .range(d3.schemeCategory10);

    // Group points by Point_ID to handle potential duplicates
    ...
    points.forEach(point => {
        ...
        pointMap.get(pointId).push(point);
    });

    // Generate paths for each unique Point_ID
    const pointPaths = [];
    pointMap.forEach((pointGroup, pointId) => {
        // Assume the first point in the group for path calculation (since Point_ID should be unique)
        const point = pointGroup[0];
        const assignments = getNodeAssignments(point, dt.root);
        const actualLabel = getActualLabel(point, options.labelsData);
        const predictedLabel = dt.predict([point])[0];

        // Create path coordinates for visualization
        const path = assignments.map(assignment => {
            const node = dt.levels[assignment.depth].find(n => n.nodeId === assignment.nodeId);
            return {
                nodeId: assignment.nodeId,
                depth: assignment.depth,
                radius: node.radius,
                angle: node.startAngle + node.angle / 2, // Center of node's angular span
                isLeaf: assignment.isLeaf,
        ...
        pointPaths.push({
            ...
            pathColor: colorScale(actualLabel),
            pointColor: colorScale(predictedLabel),
            points: pointGroup // Store all points with this Point_ID
        });
    });
    ...
    return {
        ...
        transformedPoints: points.map(point => ({
            ...
            nodeAssignments: getNodeAssignments(point, dt.root),
            nodePath: getNodePath(point, dt.root),
            pointPath: pointPaths.find(p => p.Point_ID === point.Point_ID)
        }))
    };
};

// Helper function to get actual label for a point
const getActualLabel = (point, labelsData) => {
    if (!labelsData || !labelsData.labels) return 'unlabeled';
    const pointId = point.Point_ID;
...
// Helper function to get node assignments for a point
const getNodeAssignments = (point, root) => {
    const assignments = [];
    let currentNode = root;
    let depth = 0;
