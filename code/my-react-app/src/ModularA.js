...
const HierarchicalGraph = ({
    ...
    const renderVisualization = () => {
        ...
        ringStructure.forEach((ring, index) => {
            ...
            let ringLabelText = `${ringLabels[index]} (${ring.points?.length || 0} points)`;
            if (transformStrategy === CoordinateTransforms.DECISION_TREE) {
                ringLabelText = `Depth ${index} (${ring.points?.length || 0} points)`;
            }
            ...
            // Draw decision tree connections if applicable
            if (transformStrategy === CoordinateTransforms.DECISION_TREE && ring.nodes) {
    ...
    const renderRingSectors = (
        ...
        if (transformStrategy === CoordinateTransforms.DECISION_TREE) {
            // For decision tree, we don't draw sectors since the points are already positioned
            return;
        }
    ...
    const renderLinearStrips = (ringStructure, sectorCounts) => {
        ...
        ringStructure.forEach((ring, ringIndex) => {
            ...
            let stripTitle = `Ring ${String.fromCharCode(65 + ringIndex)}: ${ring.key} (${ring.points?.length || 0} points)`;
            if (transformStrategy === CoordinateTransforms.DECISION_TREE) {
