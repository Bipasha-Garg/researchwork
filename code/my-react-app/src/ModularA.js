...
//     const [decisionTreeFullData, setDecisionTreeFullData] = useState(null);
...
//             const nodeInfo = point.nodeAssignments ? `Nodes: ${point.nodeAssignments.length}` : 'No path';
...
const HierarchicalGraph = ({
    ...
    const renderVisualization = () => {
        ...
        let ringStructure = generateRingStructure(
            jsonData,
            transformStrategy,
            transformOptions,
            labelsData
        );
        let sectorCounts = calculateSectorPointCounts(
            ringStructure,
            transformStrategy,
            transformOptions,
            labelsData
        ...
        ringStructure.forEach((ring, index) => {
            ...
            let positions = calculatePointPositions(
                ring.points || [],
                index,
                innerRadius,
                outerRadius,
        ...
        console.log("Final pointPositions:", pointPositions);

        // Draw point connections for non-decision tree strategies
        if (transformStrategy !== CoordinateTransforms.DECISION_TREE) {
            drawEnhancedConnections(g, pointPositions, getLabelColor, tooltip);
        } else {
            // Draw tree edges for decision tree
            drawTreeEdges(g, ringStructure, getLabelColor, tooltip);
        }
    ...
    const drawTreeEdges = (g, ringStructure, getLabelColor, tooltip) => {
        if (transformStrategy !== CoordinateTransforms.DECISION_TREE) return;

        const connectionsGroup = g.append("g").attr("class", "tree-edges");
        const structure = ringStructure[0]?.structure; // Assuming structure is in the first ring

        if (!structure || !structure.connections) return;

        structure.connections.forEach((connection, index) => {
            connectionsGroup.append("line")
                .attr("x1", connection.parent.centerX)
                .attr("y1", connection.parent.centerY)
                .attr("x2", connection.child.centerX)
                .attr("y2", connection.child.centerY)
                .attr("stroke", "#555")
                .attr("stroke-width", 1)
                ...
                .on("mouseover", (event) => {
                    tooltip
                        .style("visibility", "visible")
                        .html(`<strong>Tree Edge:</strong><br>
                               <strong>From Node:</strong> ${connection.parentNode.nodeId}<br>
                               <strong>To Node:</strong> ${connection.childNode.nodeId}<br>
                               <strong>Feature:</strong> ${connection.feature || 'N/A'}<br>
                               <strong>Threshold:</strong> ${connection.threshold?.toFixed(2) || 'N/A'}<br>
                               <strong>Direction:</strong> ${connection.direction}`);
